...
// Replace with the actual server port number
...
{   
    ...
    while (1)
    {
        ...
        if (strncmp(command, "findfile ", 9) == 0)
        {
            ...
            snprintf(filename, sizeof(filename), "%s", command + 9); // filename extraction from the command
            ...
            {
                response[bytes_received] = '\0'; // null terminator
        ...
        else if (strncmp(command, "sgetfiles", 9) == 0)
        {
            ...
            if (unzip_flag)
            {
                // using system call Unzip the received file
